...
async function startServer() {
    ...
    async (req: Request, res: Response) => {
      ...
      if (webhookId && receivedSig && certUrl && transmissionId) {
        try {
          ...
          if (String(process.env.PAYPAL_SANDBOX || 'true').toLowerCase() === 'false') {
  ...
  async function markOrderPaidInDb(orderId: string, extra: { amount?: number; storeTotal?: number; storeCurrency?: string; paidCurrency?: string; customer?: { name?: string; email?: string; phone?: string }; method?: string; txnId?: string } = {}): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(rows) && rows.length > 0) {
        ...
        const recStoreCcy = extra.storeCurrency || (extra.storeTotal === undefined ? recPaidCcy : undefined);
        const recovery = { orderNumber: orderId, gatewayOrderId: orderId, customerName: extra.customer?.name || '', email: (extra.customer?.email || '').toLowerCase(), phone: extra.customer?.phone || '', items: [], total: extra.storeTotal ?? extra.amount ?? 0, ...(recStoreCcy ? { currency: recStoreCcy } : {}), paidAmount: extra.amount ?? 0, ...(recPaidCcy ? { paidCurrency: recPaidCcy } : {}), paymentMethod: extra.method || 'Online', transactionId: extra.txnId || '', paymentStatus: 'Paid', orderStatus: 'Confirmed', createdAt: new Date().toISOString(), _recoveredFromIpn: true };
        ...
        console.log('[IPN] Recovery order inserted (customer did not return):', orderId);
      }
    } catch (e: any) {
  ...
  async function markOrderPaidFirebase(orderId: string, extra: { amount?: number; storeTotal?: number; storeCurrency?: string; paidCurrency?: string; customer?: { name?: string; email?: string; phone?: string }; method?: string; txnId?: string } = {}): Promise<void> {
    ...
    try {
      ...
      if (snap.exists) {
        ...
        const fbStoreCcy = extra.storeCurrency || (extra.storeTotal === undefined ? fbPaidCcy : undefined);
        await ref.set({ id: orderId, orderNumber: orderId, gatewayOrderId: orderId, customerName: extra.customer?.name || '', email: (extra.customer?.email || '').toLowerCase(), phone: extra.customer?.phone || '', items: [], total: extra.storeTotal ?? extra.amount ?? 0, ...(fbStoreCcy ? { currency: fbStoreCcy } : {}), paidAmount: extra.amount ?? 0, ...(fbPaidCcy ? { paidCurrency: fbPaidCcy } : {}), paymentMethod: extra.method || 'Online', transactionId: extra.txnId || '', paymentStatus: 'Paid', orderStatus: 'Confirmed', createdAt: new Date().toISOString(), _recoveredFromIpn: true });
        console.log('[IPN] (Firebase) Recovery order inserted (customer did not return):', orderId);
      }
    } catch (e: any) {
  ...
  app.post('/api/sslcommerz/ipn', async (req: Request, res: Response) => {
    ...
    try {
      const sandbox   = String(process.env.SSLCZ_SANDBOX ?? 'true').toLowerCase() !== 'false';
      const storeId   = String(process.env.SSLCZ_STORE_ID       || '');
      const storePass = String(process.env.SSLCZ_STORE_PASSWORD  || '');
      const base = sandbox ? 'https://sandbox.sslcommerz.com' : 'https://securepay.sslcommerz.com';
  ...
  app.all('/api/jazzcash/callback', (req: Request, res: Response) => {
    ...
    const salt = String(process.env.JAZZCASH_SALT || '').trim();
    let verified = false;
    if (rcvdHash && salt) {
      const params = { ...body };
