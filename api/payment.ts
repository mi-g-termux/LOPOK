  ...
  try {
    ...
    if (Array.isArray(rows) && rows.length > 0) {
      ...
      const recovery = {
        ...
        customerName: extra.customer?.name || '', email: (extra.customer?.email || '').toLowerCase(),
        phone: extra.customer?.phone || '', items: [],
        // total is in the STORE currency (threaded via ipn_url). Fall back to the
        // native charge only when the store total was not supplied.
        total: extra.storeTotal ?? extra.amount ?? 0,
      ...
      console.log('[IPN] Recovery order inserted (customer did not return):', orderId, ins.status);
    }
  } catch (e: any) {
...
async function persistPaidOrder(backend: string, orderId: string, extra: { amount?: number; storeTotal?: number; storeCurrency?: string; paidCurrency?: string; customer?: { name?: string; email?: string; phone?: string }; method?: string; txnId?: string } = {}): Promise<void> {
...
async function sslcommerzValidate(valId: string, sandboxOverride?: boolean): Promise<boolean> {
  ...
  try {
    ...
    // and fall back to env only when no override was provided.
    const sandbox   = sandboxOverride !== undefined ? sandboxOverride : env('SSLCZ_SANDBOX') !== 'false';
    const base = sandbox ? 'https://sandbox.sslcommerz.com' : 'https://securepay.sslcommerz.com';
...
async function paytmInitiate(req: VercelRequest, res: VercelResponse) {
  ...
  const key = body.key || env('PAYTM_MERCHANT_KEY') || env('PAYTM_KEY');
...
async function upiCreateIntent(req: VercelRequest, res: VercelResponse) {
  ...
  if (!amount || !orderId) return fail(res, 400, 'amount and orderId required');
  const upiId = body.upiId || env('UPI_VPA') || env('UPI_ID');
